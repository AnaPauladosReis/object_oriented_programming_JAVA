...
public class Controladora {
    ...
    public static void mostrarMenu() {
        ...
        do {
            ...
            opcaoMenu = Entrada.solicitarMenu(opcoes, "Papelaria Ponei");
            ...
            switch (opcaoMenu) {
                case 0: { // Registro de Produtos
                    switch (opcaoSubMenu) {
                        ...
                        case 2: { // Atualizar produto registrado
                            if (ProdutoRegistrado.verificarProdutosRegistradosVazios()) {
                                Saida.exibirNaoTem("produtos registrados");
                            } else {
                                String id = Entrada.solicitarIdProduto();
                                if (ProdutoRegistrado.verificarIdExistente(id)) {
                                    String novaDescricao = Entrada.solicitarDescricaoProduto();
                                    double novoPreco = Entrada.solicitarPrecoProduto();
                                    ProdutoRegistrado.atualizarProdutoRegistrado(id, novaDescricao, novoPreco);
                                } else {
                                    Saida.exibirNaoTem("produto com esse código");
                                }
                            }
                            break;
                        }
                        case 3: { // Remover produto registrado
                            if (ProdutoRegistrado.verificarProdutosRegistradosVazios()) {
                                Saida.exibirNaoTem("produtos registrados");
                            } else {
                                String id = Entrada.solicitarIdProduto();
                                if (ProdutoRegistrado.verificarIdExistente(id)) {
                                    ProdutoRegistrado.removerProdutoRegistrado(id);
                                } else {
                                    Saida.exibirNaoTem("produto com esse código");
                                }
                            }
                ...
                case 1: { // Estoque de Produtos
                    switch (opcaoSubMenu) {
                        ...
                        case 3: { // Atualizar produto no estoque
                            if (ProdutoEstocado.verificarProdutosEstoqueVazios()) {
                                Saida.exibirNaoTem("produtos no estoque");
                            } else {
                                String id = Entrada.solicitarIdProduto();
                                if (ProdutoEstocado.verificarIdExistenteNoEstoque(id)) {
                                    int novaQuantidade = Entrada.solicitarQuantidadeProduto("atualizar no estoque");
                                    ProdutoEstocado.atualizarProdutoNoEstoque(id, novaQuantidade);
                                } else {
                                    Saida.exibirNaoTem("produto com esse código no estoque");
                                }
                            }
                            break;
                        }
                        case 4: { // Remover produto do estoque
                            if (ProdutoEstocado.verificarProdutosEstoqueVazios()) {
                                Saida.exibirNaoTem("produtos no estoque");
                            } else {
                                String id = Entrada.solicitarIdProduto();
                                if (ProdutoEstocado.verificarIdExistenteNoEstoque(id)) {
                                    ProdutoEstocado.removerProdutoDoEstoque(id);
                                } else {
                                    Saida.exibirNaoTem("produto com esse código no estoque");
                                }
                            }
